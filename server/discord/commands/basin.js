...
const basinCommand = {
  ...
  execute: async (message, args) => {
    try {
      ...
      const pressEmbed = createPressEmbed(timing, coachName, user.currentTeam);
...
async function sendNextQuestion(message, user, sessionId) {
...
async function handlePressConferenceEnd(message, user, sessionId) {
  const session = await storage.getGameSession(sessionId);
  if (!session) return;
